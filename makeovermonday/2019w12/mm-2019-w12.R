...
reykjavik_orig <- readxl::read_xlsx("data-raw/makeovermonday/2019w12/Reykjavik Index.xlsx")
names(reykjavik_orig) <- c("country", "index")

reykjavik_average <- reykjavik_orig %>% 
  filter(country == "G7 Average")

p_wo_annotations <- reykjavik_orig %>% 
  filter(country != "G7 Average") %>% 
...
p_wo_annotations + 
  annotate(label = "Less than 30%",
           "text", colour = "grey30", x = 6, y = 80, size = 3.5, hjust = "left", fontface = 2) +
  annotate(label = "of",
           "text", colour = "grey30", x = 6, y = 94, size = 3.5, hjust = "left") +
  annotate(label = "\npeople don't think men\nand women are equally\nsuited in leadership\nroles",
           "text", colour = "grey30", x = 5.3, y = 80, size = 3.5, hjust = "left") +
  
  annotate(label = "Up to 39%",
           "text", colour = "#623460", x = 2.5, y = 80,   size = 3.5, hjust = "left", fontface = 2) +
  annotate(label = "of people",
           "text", colour = "#623460", x = 2.5, y = 90, size = 3.5, hjust = "left") +
  annotate(label = "\ndon't think men and\nwomena re equally\nsuited in leadership roles",
           "text", colour = "#623460", x = 1.9, y = 80,  size = 3.5, hjust = "left")
